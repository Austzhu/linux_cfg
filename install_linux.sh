...
if [ "$flag" == "y" -o "$flag" == "" ];then
	echo "install sublimetext and sogou..."
	#安装sublimtext
	echo "确认sublime_text安装包的路径..."
	SUBDIR=~/winshare/x64/sublime-text_build-3103_amd64.deb
	read -p "默认路径~/winshare/x64/sublimetext.deb  y?n  " flag
	if [ "$flag" != "y" ];then
		echo "输入sublime_text路径"
		read SUBDIR
	fi
	echo "你的sublime_text路径是：$SUBDIR"
	dpkg -i $SUBDIR
fi
...
if [ "$flag" == "y" -o "$flag" == "" ];then
	...
	read -p "默认路径~/winshare/soft/sublimtext/sublimetext.deb  y?n  " flag
	if [ "$flag" != "y" ];then
		echo "输入sogoupinyin路径"
		read SOGOUDIR
	fi
	echo "你的sogoupinyin路径是：$SOGOUDIR"
	dpkg -i $SOGOUDIR
fi
...
if [ "$flag" == "y" -o "$flag" == "" ];then
	#echo "export PS1='\u@Austzhu: \W\\\$'" >> ~/.bashrc
	source ~/.bashrc
	#汉化sublimetext
	#cp /mnt/hgfs/winshare/x64/Default.sublime-package ~/.config/sublime-text-3/Installed\ Packages/ -frd
	#安装ctags和cscope
