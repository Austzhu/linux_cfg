...
# 安装解决不能输入中文支持库
apt-get install build-essential libgtk4.0-dev -y
...
sed -i "s/Exec=\(.*%F\)/Exec=bash -c \"${SUBL_STRING} \1\"/"		${SUBL_DESTTOP}
sed -i "s/Exec=\(.*-n\)/Exec=bash -c \"${SUBL_STRING} \1\"/"		${SUBL_DESTTOP}
sed -i "s/Exec=\(.*new_file\)/Exec=bash -c \"${SUBL_STRING} \1\"/"	${SUBL_DESTTOP}
